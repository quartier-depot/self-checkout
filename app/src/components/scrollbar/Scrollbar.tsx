...
interface ScrollbarProps extends React.ComponentPropsWithoutRef<'div'> {
    scrollPositionAfterUpdate: 'top' | 'bottom';
    children: React.ReactNode;
}

// Source: https://www.thisdot.co/blog/creating-custom-scrollbars-with-react
const Scrollbar: React.FC<ScrollbarProps> = ({ children, className, scrollPositionAfterUpdate, ...props }) => {
    ...
    const resetScrollPosition = useCallback(() => {
        if (contentRef.current) {
            contentRef.current.scrollTop = scrollPositionAfterUpdate === 'top' ? 0 : contentRef.current.scrollHeight;
            handleThumbPosition();
        }
    }, [handleThumbPosition, scrollPositionAfterUpdate]);
    ...
    useEffect(() => {
        if (contentRef.current) {
            ...
            mutationObserver.current = new MutationObserver((mutations) => {
                ...
                // If items were added or removed, reset scroll position
    ...
    }, [handleResize, checkOverflow, resetScrollPosition]);
